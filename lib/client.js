// dsh-lan-proxy 浏览器端 bundle（手写，无构建步骤）——照 dsh-litellm-key-usage 同构。
//
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var st = {
      ...
      saveBtn: { padding: "6px 16px", borderRadius: "6px", border: "0", cursor: "pointer", fontSize: "13px", fontWeight: 600, color: "#fff", background: "var(--dsw-alias-brand-primary, #3964fe)" },
    ...
    function LanProxySection(props) {
      ...
      function Field(props) {
        return React.createElement("label", { style: st.row },
          ...
          React.createElement("input", {
            ...
            spellCheck: false,
            onChange: function (e) { setField(props.field, props.numeric ? e.target.value : e.target.value); },
          })
        );
      }
      ...
      function Checkbox(props) {
        return React.createElement("label", { style: st.row },
          React.createElement("input", { type: "checkbox", style: st.check, checked: !!props.value, disabled: !writable, onChange: function (e) { setField(props.field, e.target.checked); } }),
          React.createElement("span", { style: st.hint }, props.label)
        );
      }

      var statusNode;
      if (!ready) statusNode = React.createElement("p", { style: st.status }, "配置加载中…");
      else if (!writable) statusNode = React.createElement("p", { style: st.status }, "当前连接为只读（settings 仅回环连接可写）。");
      else if (status === "保存中…") statusNode = React.createElement("p", { style: st.status }, status);
      else if (status) statusNode = React.createElement("p", { style: st.ok }, status);
      else statusNode = React.createElement("p", { style: st.hint }, "更改后点击保存，host 侧立即热启停代理。");

      return React.createElement("div", { style: st.card },
        React.createElement("div", null,
          React.createElement("h3", { style: st.title }, "局域网反向代理 (dsh-lan-proxy)"),
          React.createElement("p", { style: st.desc }, "把本机 DSH Web（127.0.0.1:3080）通过指定 host:port 暴露到局域网，手机/其他电脑可访问。")
        ),
        React.createElement(Checkbox, { field: "enabled", label: "启用代理（关闭时不监听任何端口）", value: draft.enabled }),
        React.createElement(Field, { field: "host", label: "监听地址 (host)", value: draft.host }),
        React.createElement(Field, { field: "port", label: "监听端口 (port)", value: portText, numeric: true }),
        React.createElement("hr", { style: st.sep }),
        React.createElement(Checkbox, { field: "authEnabled", label: "启用访问认证（Basic）", value: draft.authEnabled }),
        React.createElement(Field, { field: "authUser", label: "认证用户名", value: draft.authUser }),
        React.createElement(Field, { field: "authPass", label: "认证密码", value: draft.authPass, type: "password", autoComplete: "new-password" }),
        ...
        error ? React.createElement("p", { style: st.error }, error) : null
      );
    }
