...
//
// 设置（DSH 设置面板自动渲染 dsh-lan-proxy 命名空间）：
//   enabled: 是否启动反代（默认 false —— 安装后不监听任何端口，需手动开启）
//   host:    监听地址（默认 0.0.0.0）
//   port:    监听端口（默认 15151）
//
// 重启健壮性（两端）：
// 1) 启动端：代理端口被占用（旧实例收尾未完成、或他进程占用 15151）时绑定
...
//    否则宿主优雅退出后事件循环残留 15151 监听 handle，进程挂着不退——重启
...
const ProxySchema = z.object({
  ...
  authPass: z.string().default('lan-proxy-2026'),
});
...
export function apply(ctx, config = {}) {
  ...
  async function startProxy(v, mySeq, attempt = 0) {
    ...
    try {
      ...
      proxy = p;
      logger.info(`dsh-lan-proxy: proxy ready on ${v.host}:${proxy.port} | 局域网代理已就绪 (http://<本机IP>:${proxy.port})${v.authEnabled ? ' [auth on]' : ''}`);
    } catch (err) {
      logger.error(`dsh-lan-proxy: proxy start failed on ${v.host}:${v.port} | 代理启动失败: ${err?.message ?? err}`);
      if (attempt < RETRY_DELAYS.length) {
        const delay = RETRY_DELAYS[attempt];
        logger.warn(`dsh-lan-proxy: port ${v.host}:${v.port} busy — retrying in ${delay}ms (${attempt + 1}/${RETRY_DELAYS.length}) | 端口被占用，${delay}ms 后重试`);
        retryTimer = setTimeout(() => { retryTimer = null; void startProxy(v, mySeq, attempt + 1); }, delay);
      } else {
        logger.error(`dsh-lan-proxy: gave up after ${RETRY_DELAYS.length} retries — port ${v.host}:${v.port} still in use. Free the port or change the port setting, then toggle the proxy off/on. | 重试 ${RETRY_DELAYS.length} 次后放弃，端口仍被占用。请释放端口或修改设置后重新开关代理。`);
      }
    }
  }
  ...
  // 不等代理关闭就继续，事件循环里残留 15151 的监听 handle，进程"挂着不退出"：
  // 表现为 3080 已释放、15151 仍被占（用户看到的"lan-proxy 进程没退出"），重启
