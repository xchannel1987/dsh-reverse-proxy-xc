...
export function rewriteHeaders(inHeaders) {
  ...
  // The browser marks requests from a non-loopback page as `sec-fetch-site:
  // cross-site`. After we rewrite Host/Origin to loopback the upstream DSH
  // sees the request as same-origin, but its /api trust fence still rejects
  // an explicit `cross-site` marker (browser WebSocket upgrade handshakes
  // carry it). Rewriting the marker to `same-origin` keeps the fence honest.
  if (out['sec-fetch-site'] === 'cross-site') out['sec-fetch-site'] = 'same-origin';
  return out;
}
