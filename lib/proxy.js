...
const AUTH_COOKIE = 'dsh_lan_proxy_auth';
...
/** HMAC-SHA256 签名（服务器侧秘密，进程内保持），用于认证 token。 */
function makeSigner(secret) {
  const key = createHash('sha256').update(String(secret ?? 'dsh-lan-proxy')).digest();
...
function sendUnauthorized(res) {
  ...
  res.writeHead(401, {
    ...
    'www-authenticate': 'Basic realm="dsh-lan-proxy"',
 ...
 */
export function createLanProxy({ port = 15151, host = '0.0.0.0', upstream = DEFAULT_UPSTREAM, log = null, injectHtml = RANDOM_UUID_POLYFILL, auth = null } = {}) {
  const sign = makeSigner(process.pid + ':' + process.env.DSH_HOME + ':' + randomBytes(8).toString('hex'));
  const server = createServer((req, res) => {
    ...
    if (setCookie) {
      // 首次 Basic 成功：代理前不转发（种 cookie 后立即 200），
      // 浏览器带 cookie 的下一次请求自然放行。
      res.writeHead(200, { 'set-cookie': [setCookie], 'content-type': 'text/plain', 'content-length': 2 });
      res.end('ok');
      return;
    }
  ...
  server.on('upgrade', (req, socket, head) => {
    ...
    const headers = loopbackAuthority({ ...req.headers }, upstream);
    if (gate.token) headers.cookie = `${AUTH_COOKIE}=${gate.token}`;
    const proxyReq = httpRequest({
      host: upstream.host, port: upstream.port, method: req.method, path: req.url, headers, agent: false,
    });
