...
const AUTH_SALT = 'dsh-lan-proxy:v1';
        ...
        if (isBundle && proxyRes.statusCode === 200) {
          ...
          if (isJs && !isCompressed(proxyRes.headers)) {
            ...
            proxyRes.on('end', () => {
              ...
              res.end(out);
              log?.(`[dsh-lan-proxy] patched client-connection bundle (isLoopback=true): ${originalLen} -> ${patchedLen} bytes`);
            });
            proxyRes.on('error', () => res.destroy());
            return;
          }
          log?.(`[dsh-lan-proxy] bundle skipped: contentType=${contentType}, compressed=${isCompressed(proxyRes.headers)}`);
        }
